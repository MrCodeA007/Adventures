...
let dropdownSelectorEl = document.getElementById(`dropdownselector`);
function dropdownCategories() {
  for (let i = 0; i < categories.length; i++) {
    let optionEl = document.createElement("option");
    const activitycategory = categories[i];
    optionEl.textContent = activitycategory;
    dropdownSelectorEl.appendChild(optionEl);
  }
}

dropdownCategories();
//---------------------------------------------
function oncategoryclickedChanged() {
  let selectedValue = dropdownSelectorEl.value;
    let selectedCategory = activities.length;
    for (let i = 0;i <selectedCategory;i++) {
        console.log(categories.selectedCategory[i]);
    }
};
const locationdivEl = document.getElementById(`locationDiv`)
const submitEl = document.getElementById(`submitBtn`);
submitEl.onclick = oncategoryclickedChanged;